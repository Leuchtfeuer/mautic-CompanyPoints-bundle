...
class TriggerEventController extends CommonFormController
{
    ...
    public function newAction(Request $request)
    {
        ...
        if ('POST' == $method) {
            $triggerEvent = $request->request->all()['companypointtriggerevent'] ?? [];
        ...
        if ('POST' == $method) {
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $actions            = $session->get('mautic.companypoint.'.$triggerId.'.triggerevents.modified');
                    ...
                    $session->set('mautic.companypoint.'.$triggerId.'.triggerevents.modified', $actions);
                }
            }
        }
        ...
        $passthroughVars = [
            'mauticContent' => 'companypointTriggerEvent',
            'success'       => $success,
            'route'         => false,
        ];
        ...
        if (!empty($keyId)) {
            ...
            $entity       = new CompanyTriggerEvent();
    ...
    public function editAction(Request $request, $objectId)
    {
        ...
        if (null !== $triggerEvent) {
            ...
            $form = $this->formFactory->create(CompanyTriggerEventType::class, $triggerEvent, [
                'action'   => $this->generateUrl('mautic_company_pointtriggerevent_action', ['objectAction' => 'edit', 'objectId' => $objectId]),
            ...
            if ('POST' == $method) {
                if (!$cancelled = $this->isFormCancelled($form)) {
                    if ($valid = $this->isFormValid($form)) {
                        ...
                        $session->set('mautic.companypoint.'.$triggerId.'.triggerevents.modified', $events);
    ...
    public function deleteAction(Request $request, $objectId)
    {
        ...
        $events    = $session->get('mautic.companypoint.'.$triggerId.'.triggerevents.modified', []);
        $delete    = $session->get('mautic.companypoint.'.$triggerId.'.triggerevents.deleted', []);
        ...
        if ('POST' == $request->getMethod() && null !== $triggerEvent) {
            ...
            if (!in_array($objectId, $delete)) {
                $delete[] = $objectId;
                $session->set('mautic.companypoint.'.$triggerId.'.triggerevents.deleted', $delete);
            }
