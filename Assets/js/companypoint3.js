...
Mautic.companypointTriggerOnLoad = function (container) {
    if (mQuery(container + ' #list-search').length) {
        Mautic.activateSearchAutocomplete('list-search', 'companypoint.trigger');
    }
    ...
    if (mQuery('#triggerEvents')) {
        ...
        mQuery('#triggerEvents').sortable({
            ...
            stop: function(i) {
                mQuery.ajax({
                    type: "POST",
                    url: mauticAjaxUrl + "?action=plugin:LeuchtfeuerCompanyPoints:reorderTriggerEvents",
                    data: mQuery('#triggerEvents').sortable("serialize") + "&triggerId=" + mQuery('#companypointtrigger_sessionId').val()
                });
            }
        });
...
Mautic.getCompanyPointActionPropertiesForm = function(actionType) {
    Mautic.activateLabelLoadingIndicator('point_type');

    var query = "action=plugin:LeuchtfeuerCompanyPoints:getActionForm&actionType=" + actionType;
    mQuery.ajax({
        url: mauticAjaxUrl,
        type: "POST",
        data: query,
...
Mautic.EnablesCompanyOption = function (urlActionProperty) {
